...
import { updateRecorder } from "../services/recorder";

function RecorderModifyModal({ close,recorderEdit, updateRecorderList, hardwareList }) {
  const [isLoading, setLoading] = useState(false);
  const [recorder, setRecorder] = useState(recorderEdit);
  const handleInput = (e) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    const { status, data, errorMessage } = await updateRecorder(recorder);
    setLoading(false);
    if (!status) {
      toast.error(errorMessage);
      return;
    }
    updateRecorderList(recorder);
    toast.success("Grabadora actualizada!");
  };
...
export default RecorderModifyModal;
